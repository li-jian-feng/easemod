...
$pwd = 'aaaaaa'; // 密码
/*
 * 注册用户 $acc_add = $easemob->postRequest ( 'https://a1.easemob.com/' . $org_name . '/' . $app_name . '/users', '{"username":"' . $uid . '","password":"' . $pwd . '"}', $access_token); $acc_add = json_decode ( $acc_add, true ); print_r ( $acc_add ); $fuid='bbbbbb'; //好友用户名 //添加用户关系 这里参数{"username":"aaaaaa1","password":"aaaaaa1"} 为用户管理员账号密码 ,需要后台手动添加 $acc_contact = $easemob->postRequest ( 'https://a1.easemob.com/' . $org_name . '/' . $app_name . '/users/' . $uid . '/contacts/users/' . $fuid, '{"username":"aaaaaa","password":"aaaaaa"}', $access_token); $acc_contact = json_decode ( $acc_contact, true ); print_r ( $acc_contact ); //批量删除用户 $acc_delete = $easemob->postRequest ( 'https://a1.easemob.com/' . $org_name . '/' . $app_name . '/users?limit=300', '', $access_token ,'DELETE'); $acc_delete = json_decode ( $acc_delete, true ); print_r ( $acc_delete ); //删除指定用户 $acc_delete_by_uid = $easemob->postRequest ( 'https://a1.easemob.com/' . $org_name . '/' . $app_name . '/users/' . $uid, '', $access_token, 'DELETE' ); $acc_delete_by_uid = json_decode ( $acc_delete_by_uid, true ); print_r ( $acc_delete_by_uid );
 */
$fuid = 'bbbbbb';
// 好友用户名
